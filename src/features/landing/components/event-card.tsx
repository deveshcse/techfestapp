...
export function EventCard({ title, description, date, location, category, image }: EventCardProps) {
    return (
        <Card className="group relative overflow-hidden border-border/10 bg-accent/5 transition-all duration-300 hover:border-cyan-500/50 hover:shadow-[0_0_30px_-5px_hsl(var(--primary)/0.3)]">
            ...
            <CardHeader className="p-0">
                <div className="relative h-48 w-full overflow-hidden">
                    ...
                    <div className="absolute left-4 top-4">
                        <Badge variant="secondary" className="bg-background/50 text-foreground backdrop-blur-md hover:bg-background/70">
                            {category}
                        </Badge>
                    </div>
                </div>
            ...
            <CardContent className="relative z-10 space-y-4 p-6">
                <div>
                    <h3 className="text-xl font-bold text-foreground group-hover:text-cyan-400 transition-colors line-clamp-1">
                        {title}
                    </h3>
            ...
            <CardFooter className="relative z-10 p-6 pt-0">
                <Button className="w-full bg-accent text-foreground hover:bg-accent/20 hover:text-cyan-400" asChild>
                    <Link href="#">
                        View Details
