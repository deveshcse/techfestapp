...
const tabs = productShowcase.tabs;
/** Scroll distance per tab — higher = longer dwell before the next tab */
const ITEM_VH = 85;
...
export function ProductShowcase() {
  ...
  const active = tabs[activeIndex]?.id ?? tabs[0].id;
  ...
  const selectTab = (id: string) => {
    ...
    if (!enableParallax || !trackRef.current) return;

    const track = trackRef.current;
    const trackTop = track.getBoundingClientRect().top + window.scrollY;
    const range = Math.max(0, track.offsetHeight - window.innerHeight);
    const targetY = trackTop + (index / tabs.length) * range;
    ...
    >
      ...
      {enableParallax ? (
        <div
          ref={trackRef}
          className="relative"
          style={{ height: `${tabs.length * ITEM_VH}vh` }}
        >
          <div className="sticky top-16 flex h-[calc(100vh-4rem)] flex-col justify-center px-(--landing-gutter)">
            {tablist}
            {panel}
            <p
              className="mt-(--landing-stack-md) text-center font-landing-body text-xs tabular-nums text-landing-ink-muted"
              aria-live="polite"
            >
        ...
        <motion.div
          className="mt-(--landing-stack-lg) w-full px-(--landing-gutter)"
          initial={prefersReducedMotion ? false : "hidden"}
          whileInView="visible"
          viewport={viewport}
          variants={fadeUp}
