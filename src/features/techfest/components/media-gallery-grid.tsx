...
import { FileText, Play, Trash2, Maximize2, Video as VideoIcon } from "lucide-react";
...
export function MediaGalleryGrid({ media, onDelete, editable = false }: MediaGalleryGridProps) {
                        ...
                        <CardContent className="p-0 relative aspect-square bg-muted">
                            ...
                            {item.type === MediaType.VIDEO && (
                                <div className="w-full h-full flex items-center justify-center bg-slate-900 border overflow-hidden relative">
                                    {item.thumbnail ? (
                                        <Image src={item.thumbnail} alt="Video thumbnail" fill className="object-cover opacity-60" />
                                    ) : (
